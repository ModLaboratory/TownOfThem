...
namespace TownOfThem.CredentialsPatches
{
    [HarmonyPatch]
    public class MainMenuButtonPatches
    {
        ...
        public static void Start_Prefix(MainMenuManager __instance)
        {
            ...
            if (howToPlayButton != null)
            {
                ...
                __instance.StartCoroutine(Effects.Lerp(0.01f, new Action<float>((p) => howToPlayButton.transform.GetChild(0).GetComponent<TMPro.TMP_Text>().SetText(TownOfThem.Language.Language.LoadTranslation("Bilibili")))));
     ...
     class ModOtherInfosPatch
    {
        static void Postfix(VersionShower __instance)
        {
            ...
            {
                credentials.text = TownOfThem.Language.Language.LoadTranslation("totBirthday")+$"{TownOfThem.Main.ModName}!\nv{TownOfThem.Main.ModVer}\n{TownOfThem.Language.Language.Translation["ModInfo1"]}";
            }
            else
            {
                credentials.text = $"{TownOfThem.Main.ModName}\nv{TownOfThem.Main.ModVer}\n" + TownOfThem.Language.Language.LoadTranslation("ModInfo1");
            }
